...
#aws_url = "http://35.178.170.58/ # nginx listening on port 80
url = "http://0.0.0.0:80/"
red_X_test = util.load_red_test_data()
# TESTING ON SVD DATA
x_i, index = util.randomise(1, red_X_test)
same_index = 5
same_email = red_X_test[same_index]
...
def main() -> None:
    ...
    try:
        ...
        response = requests.post(url + "spamfilter/ts", files=data)
...
# just testing if the same email continously gives the same score
def _ts_data_to_send_static() -> tuple:
    # not lean. just testing because i'm getting different output scores for the same email?? 
    ctx = client_code.setup_lean_ts_params()
    enc_x_i = client_code.ts_encrypt_x_i(same_email, ctx)
