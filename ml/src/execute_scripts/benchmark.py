...
def test_model_latency(model_name: str) -> None:
    ...
    mean, sd, var, n = get_stats(inference_times)
    print_stats(mean, sd, var, n)
    err = calc_error_margin(sd, n)

    mean_low, mean_high = get_mean_range(mean, err)

    print(f"Inference mean: {mean_low}ms-{mean_high}ms, 95% CI")


# cpu and mem
def test_optimised_ts_server_usage() -> None:
    ...
    mean, sd, var, n = get_stats(adjusted)
    print_stats(mean, sd, var, n)
    err = calc_error_margin(sd, n)
...
def test_enc_dec_times(model_name: str) -> None:
    print(f"Testing {model_name}")
    ...
    print(enc_times)
    # ENCRYPTION STATS
    enc_mean, enc_sd, enc_var, enc_n = get_stats(enc_times)
    print_stats(enc_mean, enc_sd, enc_var, enc_n)
    enc_err = calc_error_margin(enc_sd, enc_n)

    enc_mean_low, enc_mean_high = get_mean_range(enc_mean, enc_err)

    print(f"{model_name} Encryption mean: {enc_mean_low}ms-{enc_mean_high}ms, 95% CI\n")

    print(dec_times)
    # DECRYPTION STATS
    dec_mean, dec_sd, dec_var, dec_n = get_stats(dec_times)
    print_stats(dec_mean, dec_sd, dec_var, dec_n)
    dec_err = calc_error_margin(dec_sd, dec_n)

    dec_mean_low, dec_mean_high = get_mean_range(dec_mean, dec_err)
...
def ts_test_accuracy(model_name: str) -> None:
    ...
    ):  # potentially make more robust. What if it's neither?
        is_log = False
...
def _pal_accuracy_test_loop(model, private_key, enc_x, y, is_log) -> float:
    correct_counter = 0

    for i in range(sample_size):
        enc_prelim_y = [
            model.enc_prelim_predict(enc_x[i])
        ]  # it's expecting y to be in a list (ts does this automatically. Pal doesn't)
...
def print_stats(mean, sd, var, n) -> None:
    print("Data size = ", n)
    print("Mean = ", mean)
...
def calc_error_margin(sd: float, n: int) -> float:
...
def get_mean_range(mean: float, err: float) -> tuple[float, float]:
    low = mean - err
    high = mean + err
...
def zama_benchmark_model(model_name: str) -> list:
    model = z.load_model(model_name)
    model.load()

    inference_times = _zama_inference_test_loop(
        model, model_name, _is_svd_model(model_name)
    )
...
def _zama_enc_dec_timers(model, x, cli, eval_keys, i) -> tuple:
    ...
    enc_x = cli.quantize_encrypt_serialize(
        x[i : i + 1]
    )  # i:i+1 preserves (1,n) 2d-ness
    ...
    plain_x = cli.deserialize_decrypt_dequantize(
        enc_result
    )  # not sure if decrypting without the enc_x changing is too fast? (like no server prediction to change result)
...
def _pal_enc_dec_timers(model, x, pub_key, priv_key, i) -> tuple[float, float]:
    s1 = time.perf_counter()
...
def zama_bench_plain_inference(model_name: str) -> list:
    ...
    for i in range(len(X_rand)):
        inference_times.append(zama_plain_inference(model, x[i : i + 1]))
...
def ts_bench_plain_inference(model_name: str) -> list:
    ...
    for i in range(len(X_rand)):
        inference_times.append(ts_plain_inference(model, x[i]))
        print(f"Inference {i} complete.")
...
def pca_reduce_email(pca, vectorised_email):
...
def ts_inference_time_svm(model, enc_x_i) -> float:
    start = time.perf_counter()
    enc_prelim_y = model.enc_prelim_predict(
        enc_x_i
    )  # client does the decrypt and determining sign, so this is all that's needed
...
def ts_plain_inference(model, x_i) -> float:
    start = time.perf_counter()
...
def _is_svd_model(model_name: str) -> bool:
    return model_name[:3] == "svd"


def _zama_inference_test_loop(model, model_name: str, is_pca_model) -> list:
    cli, eval_keys = z.client(model_name)
    inference_times = []
...
def _ts_or_pal_inference_test_loop(model, enc_x) -> list:
    inference_times = []
    for i in range(sample_size):
        start = time.perf_counter()
    ...
    return inference_times[
        3:
    ]  # dropping first 1 only for pal, since encryption is way too long..
