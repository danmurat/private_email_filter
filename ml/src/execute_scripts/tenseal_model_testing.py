...
t_X_train, t_y_train, t_X_test, t_y_test, t_red_X_train, t_red_X_test = (
    util.convert_to_torch_tensors(
        X_train, y_train, X_test, y_test, reduced_X_train, reduced_X_test
    )
)
...
def test_enc_svm() -> None:
    ...
    ctx_eval = client.setup_ts_params()
    ...
    enc_prelim_y = svm.enc_prelim_predict(
        enc_X_i
    )  # why are we getting different prelim values each time?
    # prelim_y = svm.enc_prelim_predict(spam_email_vector) # unencrypted prelim calc gives us same value each time (1.9 something)
    prelim_y = (
        enc_prelim_y.decrypt()
    )  # client decrypts (1.9 something too now, though it's an approximation)
...
def _setup_ts_params_svm() -> Context:
    # from tenseal docs:
    # parameters
    poly_mod_degree = (
        2**14
...
def test_lr() -> None:
    lr = t.train_log(t_X_train, t_y_train, 3000)

    t.logAccuracy(lr, t_X_test, t_y_test)


def test_pca_lr() -> None:
    pca_lr = t.train_log(t_red_X_train, t_y_train, 3000)
...
def test_svm() -> None:
    ...
    svm = t.train_svm(t_X_train, t_y_train)
