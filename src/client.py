...
def pal_gen_keys() -> tuple:
    return paillier.generate_paillier_keypair(
        n_length=2048
    )  # 256 lowest we can go without runtime errors. But is this even secure?


def pal_enc_x_i(public_key, x_i) -> list:
    return [public_key.encrypt(n) for n in x_i]


def pal_enc_dataset(public_key, x) -> list[list]:
    enc_x = []
...
def pal_decrypt_prelim(private_key, enc_prelim_y) -> list:
